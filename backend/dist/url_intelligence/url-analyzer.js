...
exports.urlIntelligenceService = exports.UrlIntelligenceService = void 0;
class UrlIntelligenceService {
    ...
    checkIpBasedUrl(parsed, heuristics) {
        if (!parsed.is_ip_based)
            return [];
    ...
    checkSuspiciousTld(parsed, heuristics) {
        const results = [];
    ...
    checkLoginKeywords(parsed, heuristics) {
        const results = [];
    ...
    checkExcessiveEncoding(url, heuristics) {
        const results = [];
    ...
    checkHomoglyphAttack(parsed, heuristics) {
    ...
    checkSuspiciousPort(parsed, heuristics) {
        if (!parsed.has_port)
            return [];
    ...
    checkSuspiciousQueryParams(parsed, heuristics) {
        const results = [];
    ...
    checkSubdomainHeuristics(parsed, heuristics) {
        const results = [];
    ...
    checkCredentialHarvesting(parsed, heuristics) {
        const results = [];
    ...
    checkPathTraversal(parsed, heuristics) {
    ...
    checkBrandInDomain(parsed, heuristics) {
        const hostname = parsed.hostname.toLowerCase();
    ...
    classifyThreat(phishingProb, riskScore, indicators) {
        if (phishingProb >= 0.8)
            return 'phishing-url';
        if (riskScore >= 60 && indicators.some(i => i.type === 'credential_harvesting_path'))
    ...
    generateSummary(url, riskLevel, probability, heuristics) {
...
exports.UrlIntelligenceService = UrlIntelligenceService;
exports.urlIntelligenceService = new UrlIntelligenceService();
exports.default = exports.urlIntelligenceService;
