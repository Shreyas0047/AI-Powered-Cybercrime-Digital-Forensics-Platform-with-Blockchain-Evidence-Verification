import { DocumentFinding, MacroRisk } from './types';

export class DocxAnalyzer {
  analyze(docxText: string, filename: string): {
    ...
    const suspiciousVbaStrings: string[] = [];
  ...
  private calculateMacroRiskScore(hasMacro: boolean, autoExec: boolean, suspiciousCount: number): number {
    let score = 0;
    if (hasMacro) score += 20;
    if (autoExec) score += 35;
