...
export class ThreatProfiler {
  matchProfile(
    features: ExtractedFeatures,
    behaviors: BehaviorFinding[]
  ): ThreatProfileMatch {
    const scores: Record<ThreatProfile, number> = {} as Record<ThreatProfile, number>;

    for (const profileDef of PROFILE_DEFINITIONS) {
      let score = 0;

      for (const [feature, weight] of Object.entries(profileDef.scoring_weights)) {
        const value = (features as any)[feature] as number;
        score += value * weight;
      }
    ...
    let bestProfile: ThreatProfile = 'benign-profile';
    let bestScore = -1;

    for (const [profile, score] of Object.entries(scores)) {
      if (score > bestScore) {
        bestScore = score;
        bestProfile = profile as ThreatProfile;
      }
    }
  ...
  private countBehaviorMatches(behaviors: BehaviorFinding[], expected: string[]): number {
  ...
  private getMatchedBehaviors(behaviors: BehaviorFinding[], expected: string[]): string[] {
  ...
  private getMissingBehaviors(behaviors: BehaviorFinding[], expected: string[]): string[] {
    const present = new Set(
      behaviors.map(b => b.behaviorType.replace(/_/g, ' ').toLowerCase())
    );
