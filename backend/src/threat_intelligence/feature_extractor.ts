...
import { NormalizedEvent, ExtractedFeatures, NormalizedEventType } from './threat_models';
...
export class FeatureExtractor {
  extractFeatures(events: NormalizedEvent[]): ExtractedFeatures {
    if (events.length === 0) {
    ...
    const fileModifyEvents = events.filter(e =>
      e.normalizedType === NormalizedEventType.FILE_MODIFY
    );
    const fileCreateEvents = events.filter(e =>
      e.normalizedType === NormalizedEventType.FILE_CREATE
    );
    const fileDeleteEvents = events.filter(e =>
      e.normalizedType === NormalizedEventType.FILE_DELETE
    );
    const fileRenameEvents = events.filter(e =>
      e.normalizedType === NormalizedEventType.FILE_RENAME
    );
    ...
    const suspiciousExtensions = allFilePaths.filter(path =>
      SUSPICIOUS_EXTENSIONS.some(ext => path.toLowerCase().endsWith(ext))
    ).length;

    const registryEvents = events.filter(e =>
      e.normalizedType === NormalizedEventType.REGISTRY_MODIFY ||
      e.normalizedType === NormalizedEventType.REGISTRY_CREATE
    );
    const registryDeleteEvents = events.filter(e =>
      e.normalizedType === NormalizedEventType.REGISTRY_DELETE
    );

    const persistenceKeys = registryEvents.filter(e =>
      e.behavioralTags.includes('persistence_registry') ||
      e.behavioralTags.includes('startup_persistence')
    );

    const networkConnectEvents = events.filter(e =>
      e.normalizedType === NormalizedEventType.NETWORK_CONNECT
    );
    const networkSendEvents = events.filter(e =>
      e.normalizedType === NormalizedEventType.NETWORK_DATA_SENT
    );
    ...
    const suspiciousPorts = [...uniquePorts].filter(port => 
      SUSPICIOUS_PORTS.includes(port as number)
    );
    ...
    const privilegeEscalationEvents = events.filter(e => {
      ...
      return PRIVILEGE_ESCALATION_PATTERNS.some(pattern => 
        processName.includes(pattern) || path.includes(pattern)
      );
    });
    ...
    const suspiciousBehaviors = events.filter(e =>
      e.behavioralTags.includes('derived_behavior') ||
      e.normalizedType === NormalizedEventType.MASS_FILE_MODIFICATION ||
      e.normalizedType === NormalizedEventType.PERSISTENCE_ATTEMPT ||
      e.normalizedType === NormalizedEventType.SUSPICIOUS_NETWORK_ACTIVITY ||
      e.normalizedType === NormalizedEventType.RAPID_PROCESS_SPAWNING ||
      e.normalizedType === NormalizedEventType.RANSOM_NOTE_CREATION
    );
    ...
    return {
      ...
      outboundConnectionCount: networkConnectEvents.filter(e => 
        e.behavioralTags.includes('external_network')
      ).length,
      inboundConnectionCount: networkConnectEvents.filter(e => 
        e.behavioralTags.includes('internal_network')
      ).length,
      uniqueDestinationIPs: [...uniqueIPs] as string[],
      uniqueDestinationPorts: [...uniquePorts] as number[],
      suspiciousPortsUsed: suspiciousPorts as number[],
  ...
  private calculateSpawnRate(events: NormalizedEvent[], duration: number): number {
    if (events.length < 2 || duration === 0) return 0;
    return (events.length / duration) * 1000; 
  }

  private estimateProcessTreeDepth(events: NormalizedEvent[]): number {
