...
export class ThreatClassifier {
  ...
  private evaluateDetectionRules(features: ExtractedFeatures): RuleMatch[] {
  ...
  private evaluateCondition(features: ExtractedFeatures, condition: RuleCondition): boolean {
    const fieldValue = (features as any)[condition.field];
  ...
  private calculateCategoryScores(triggeredRules: RuleMatch[], features: ExtractedFeatures): Record<string, number> {
  ...
  private determineThreatType(
    ...
    let maxScore = 0;
    let topCategory: ThreatClassification = 'unknown';

    for (const [category, score] of Object.entries(categoryScores)) {
      if (score > maxScore) {
        maxScore = score;
        topCategory = category as ThreatClassification;
      }
    }
  ...
  private calculateConfidence(
    ...
    if (maxScore > 50) confidence += 25;
    else if (maxScore > 30) confidence += 15;
    else if (maxScore > 15) confidence += 10;

    if (features.totalEvents > 100) confidence += 10;
    else if (features.totalEvents > 50) confidence += 5;

    if (features.suspiciousBehaviorCount > 3) confidence += 15;
    else if (features.suspiciousBehaviorCount > 0) confidence += 5;

    return Math.min(99, Math.max(0, confidence));
  }

  buildExecutionChain(events: NormalizedEvent[]): ExecutionChainStep[] {
    const sortedEvents = [...events].sort((a, b) =>
      new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
    );

    const chain: ExecutionChainStep[] = [];
    let step = 1;

    const significantEvents = sortedEvents.filter(e =>
      e.normalizedType.includes('process') ||
  ...
  private describeEventAction(event: NormalizedEvent): string {
    const type = event.normalizedType;

    if (type.includes('process_start')) return 'Process Started';
    if (type.includes('file_create')) return 'File Created';
    if (type.includes('file_rename')) return 'File Renamed';
    if (type.includes('file_delete')) return 'File Deleted';
    if (type.includes('registry')) return 'Registry Modified';
    if (type.includes('network_connect')) return 'Network Connection';
    if (type.includes('network_data_sent')) return 'Data Exfiltrated';
    if (type.includes('mass_file_modification')) return 'Mass File Operation';
    if (type.includes('persistence_attempt')) return 'Persistence Established';
    if (type.includes('ransom_note')) return 'Ransom Note Created';

    return 'System Activity';
  }

  private getEventDetails(event: NormalizedEvent): string {
    const path = event.metadata.path || event.metadata.target || '';
    const dest = event.metadata.destination;
    const port = event.metadata.port;

    if (path) return path.substring(0, 80);
    if (dest && port) return `${dest}:${port}`;
    if (dest) return dest;

    return event.normalizedType;
  }

  extractMitreTactics(techniques: string[]): string[] {
