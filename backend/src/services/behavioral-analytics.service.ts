...
import { Evidence, Investigation, TelemetryEvent } from '../models';
...
export class BehavioralAnalyticsService {
  private readonly behavioralPatterns: BehavioralPattern[] = [
    {
   ...
   */
  async analyzeProcessBehavior(evidenceId: string): Promise<ProcessBehavior> {
    ...
    for (const event of telemetry) {
      if (event.eventType.includes('network')) networkConnections++;
      if (event.eventType.includes('registry')) registryWrites++;
      if (event.eventType.includes('file')) fileOperations++;
   ...
   */
  async detectAnomalies(evidenceId: string): Promise<Anomaly[]> {
    ...
    const telemetry = await TelemetryEvent.find({ evidenceId }).limit(100).lean();
   ...
   */
  private analyzeBehavioralAnomalies(telemetry: any[]): Anomaly[] {
    const anomalies: Anomaly[] = [];
   ...
   */
  private matchPattern(event: any, pattern: BehavioralPattern): boolean {
   ...
   */
  private checkPatternMatch(evidence: any, telemetry: any[], pattern: BehavioralPattern): string[] {
   ...
   */
  private createAnomalyFromPattern(pattern: BehavioralPattern, evidenceId: string, matches: string[]): Anomaly {
   ...
   */
  private categorizeEventType(eventType: string): BehavioralCategory {
    if (eventType.includes('registry')) return BehavioralCategory.REGISTRY_ACTIVITY;
    if (eventType.includes('network') || eventType.includes('connection')) return BehavioralCategory.NETWORK_ACTIVITY;
    if (eventType.includes('file')) return BehavioralCategory.FILESYSTEM_BEHAVIOR;
    if (eventType.includes('process')) return BehavioralCategory.PROCESS_EXECUTION;
    if (eventType.includes('command') || eventType.includes('cmd')) return BehavioralCategory.COMMAND_EXECUTION;
   ...
   */
  async analyzeBehavioralBaseline(investigationId: string): Promise<{
    baseline: Record<string, { count: number; avgInterval: number }>;
    deviations: string[];
  }> {
    ...
    const baseline: Record<string, { count: number; avgInterval: number }> = {};
...
// Fuzzy IOC matching and campaign discovery
// ============================================================
...
export interface CampaignResult {
  ...
  fuzzyMatches: Array<{ type: string; a: string; b: string; similarity: number }>;
  confidence: number;
}

export class CampaignCorrelationEngine {
  /**
   * Correlate investigations by fuzzy IOC matching and shared MITRE techniques.
   */
  async correlateInvestigations(investigationIds: string[]): Promise<CampaignResult[]> {
    const investigations = await Investigation.find({ _id: { $in: investigationIds } }).lean();
    const iocs = await IOC.find({ linkedInvestigations: { $in: investigationIds } }).lean();

    // Group IOCs by investigation
    const invIocs: Record<string, typeof iocs> = {};
    for (const ioc of iocs) {
      for (const invId of (ioc as any).linkedInvestigations || []) {
        if (!invIocs[invId]) invIocs[invId] = [];
    ...
    for (let i = 0; i < investigationIds.length; i++) {
      if (visited.has(investigationIds[i])) continue;

      const cluster: string[] = [investigationIds[i]];
      const fuzzyMatches: CampaignResult['fuzzyMatches'] = [];
      const sharedTechniques = new Set<string>();

      for (let j = i + 1; j < investigationIds.length; j++) {
        if (visited.has(investigationIds[j])) continue;

        const matches = this.fuzzyMatchIocs(
          invIocs[investigationIds[i]] || [],
          invIocs[investigationIds[j]] || []
        );
      ...
      if (cluster.length >= 2) {
        ...
        for (const ev of telemetry) {
          const invId = (ev as any).investigationId;
          const tech = (ev as any).mitreTechnique;
          if (tech && invId) {
            if (!techMap[tech]) techMap[tech] = new Set();
            techMap[tech].add(invId);
          }
        }

        for (const [tech, invs] of Object.entries(techMap)) {
          if (invs.size >= 2) sharedTechniques.add(tech);
        }

        campaigns.push({
          campaignId: `CAMPAIGN-${uuidv4().slice(0, 8).toUpperCase()}`,
   ...
   */
  private fuzzyMatchIocs(aIocs: any[], bIocs: any[]): CampaignResult['fuzzyMatches'] {
    const matches: CampaignResult['fuzzyMatches'] = [];

    for (const a of aIocs) {
      for (const b of bIocs) {
        if (a._id?.toString() === b._id?.toString()) continue;
   ...
   */
  async discoverCampaigns(): Promise<CampaignResult[]> {
...
export const campaignCorrelationEngine = new CampaignCorrelationEngine();
