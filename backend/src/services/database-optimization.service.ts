...
export class DatabaseIndexManager {
   ...
   */
  registerIndexes(definition: IndexDefinition): void {
    this.indexes.push(definition);
    logger.debug(`Registered index definition for ${definition.collection}`);
  }
  ...
  async createAllIndexes(): Promise<void> {
    for (const def of this.indexes) {
      try {
        const collection = mongoose.connection.collection(def.collection);
        for (const [name, index] of Object.entries(def.indexes)) {
          const keys = typeof index === 'object' ? index : { [name]: index };
          await collection.createIndex(keys as any, { ...(def.options || {}), name } as any);
        }
        logger.info(`Created indexes for ${def.collection}`);
      } catch (error) {
        logger.error(`Failed to create indexes for ${def.collection}`, {
...
export class QueryOptimizer {
   ...
   */
  static createProjection<T>(fields: (keyof T)[]): Record<string, 1> {
   ...
   * Build sort option
   */
  static buildSortOption(sortBy?: string, order?: 'asc' | 'desc') {
   ...
   */
  static buildTextSearchQuery(query: string, fields: string[]) {
    if (!query) return {};
...
export async function checkDatabaseHealth(): Promise<{
  ...
  try {
    ...
    for (const coll of collections || []) {
      const indexes = await mongoose.connection.collection(coll.name).indexes();
...
export function getConnectionPoolStats() {
  const conn = mongoose.connections[0] as any;
